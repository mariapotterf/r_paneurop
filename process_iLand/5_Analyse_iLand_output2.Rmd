...
in Sapling table: counts_ha and count_small_ha are calculated from teh n_represented from teh  

# Example: connect tables between ILand output formats, to get the all 3 vertiocal classes.
# the regeneration 
...
```{r data_handling}
# correct, there is 5 structural clusters in climate cluster 2, and 2 clusters in clkimate cluster 3
...
big_data_sub <- big_data %>%
  dplyr::filter(year == 0 & cluster == "1_1" & clim_model == "NCC" & clim_scenario == "HISTO" & run_nr == 1 ) # & ext_seed == 'noseeed'
...
big_sapD_sub <- 
  big_sapD %>% 
  dplyr::filter(year == 0 & cluster == "1_1"& clim_model == "NCC"  & clim_scenario == "HISTO"& ext_seed == 'noseed' & run_nr == 1 ) #   & clim_scenario == "HIST0"
View(big_sapD_sub)
#big_data_sub <- 
...
# Start with the cohorts: spllit into vertical classes (seedlings (<0.2m), saplings (0.2-2m), juveniles 2m-4m)
# sum up by vertical classes, get counst_ha and dbh for basal area
