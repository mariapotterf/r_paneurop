...
-   sapling: \<4 m - from output 'saplings' -- 1.3, - 4 m - has dbh -- \<1.3m - does not have dph, organized as 'cohorts' - from output 'saplings details'
...
```{r export-table}
df_simulated_merge <- rbind(out_sapD, out_big)

# aggregate values based on vert category
df_simulated <- df_simulated_merge %>% 
  group_by(year, species, clim_model, clim_scenario, ext_seed, run_nr, cluster, category) %>% 
...
df_simulated %>% 
  dplyr::filter(cluster == "3_2" & year == 1) %>% 
  summary()
